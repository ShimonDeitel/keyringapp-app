            ...
            ZStack {
                ...
                if store.entries.isEmpty {
                    VStack(spacing: 12) {
                        ...
                        Text("Tap + to log your first key.")
                            .font(Theme.font(15))
...
struct EntryFormView: View {
    ...
    let existingID: UUID?
    let onSave: (KeyEntry) -> Void
    @FocusState private var focusedField: Field?

    enum Field { case title, detail1, detail2, note }

    init(entry: KeyEntry?, onSave: @escaping (KeyEntry) -> Void) {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(existingID == nil ? "Add Key" : "Edit Key")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = KeyEntry(title: title, detail1: detail1, detail2: detail2, note: note, date: date)
